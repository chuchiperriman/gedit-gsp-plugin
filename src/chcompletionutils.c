 ...
 */
gboolean
ch_completion_is_separator(const gunichar ch)
{
 ...
 */
gchar *
ch_completion_get_word_iter (GtkSourceBuffer *source_buffer, 
                                           GtkTextIter     *current,
					   ...
					   GtkTextIter     *end_word)
{
	...
	*end_word = *start_word;
	...
	while ((no_doc_start = gtk_text_iter_backward_char (start_word)) == TRUE)
	{
		...
		if (ch_completion_is_separator (ch))
		{
			break;
		}
	}
...
/**
 * ch_completion_get_word:
 * @source_buffer: The #GtkSourceBuffer
 *
 * Returns: the current word
 */
gchar *
ch_completion_get_word (GtkSourceBuffer *source_buffer)
{
	GtkTextIter start;
	GtkTextIter end;
	
	return ch_completion_get_word_iter (source_buffer, NULL, &start, &end);
}
...
void
ch_completion_replace_word (GtkSourceBuffer *source_buffer,
					  GtkTextIter     *iter,
					  const gchar     *text,
					  gint             len)
{
	...
	word = ch_completion_get_word_iter (source_buffer, iter, &word_start, &word_end);
	g_free (word);
...
/**
 * ch_completion_replace_current_word:
 * @source_buffer: The #GtkSourceBuffer
 ...
 */
void
ch_completion_replace_current_word (GtkSourceBuffer *source_buffer, 
						  const gchar     *text,
						  gint             len)
{
	GtkTextIter iter;
	...
	ch_completion_replace_word (source_buffer,
	                                          &iter,
	                                          text,
	                                          len);
}
 ...
 */
void
ch_completion_move_to_iter (GtkWindow     *window,
					  GtkSourceView *view,
					  GtkTextIter   *iter)
{
	gint x;
...
/**
 * ch_completion_get_pos_at_cursor:
 * @window: the #GtkWindow to move
 * @view: the view 
 *
 */
void 
ch_completion_move_to_cursor (GtkWindow     *window,
					    GtkSourceView *view)
{
	GtkTextBuffer *buffer;
	GtkTextIter insert;
	...
	ch_completion_move_to_iter (window,
	                                          view,
	                                          &insert);
}
